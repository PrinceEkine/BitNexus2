...
const AuthModal = ({ isOpen, onClose, type }: { isOpen: boolean, onClose: () => void, type: 'login' | 'signup' }) => {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Login successful! Welcome to BitNexus.');
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Signup successful! A verification email has been sent to your address. Please verify your email to continue.');
      onClose();
    } catch (err: any) {
      setError(err.message);
    } finally {
